...
# Split the top pane horizontally -> pane 2: mission supervisor NODE
tmux split-window -h -t $SESSION_NAME:0.0
tmux send-keys -t $SESSION_NAME "source $HOME/virtual_env/bin/activate; ros2 run mission_supervisor mission_supervisor_node" C-m

# Split again -> pane 3: lap counter (publishes /laps for the dynamic missions)
tmux split-window -v -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "source $HOME/virtual_env/bin/activate; ros2 run mission_supervisor lap_counter" C-m

# Select top-left pane (optional)
tmux select-pane -t $SESSION_NAME:0.0

echo "[$(date)] tmux session $SESSION_NAME started with 4 panes." >> "$LOGFILE"
echo "[$(date)] Pane 0: ackermann_can" >> "$LOGFILE"
echo "[$(date)] Pane 1: sensor_bringup.launch.py" >> "$LOGFILE"
echo "[$(date)] Pane 2: mission_supervisor_node" >> "$LOGFILE"
echo "[$(date)] Pane 3: lap_counter" >> "$LOGFILE"
