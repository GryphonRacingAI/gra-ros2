...
class LapMissionHandler(MissionHandler):
    ...
    # The car starts BEHIND the start/finish gate, so its launch run through the
    # gate is counted by lap_counter as a crossing before any lap is completed.
    # N real laps therefore means N+1 crossings, hence the +1 below. Without it
    # trackdrive finished after 9 laps and autocross fired on the launch itself.
    LAUNCH_CROSSING = 1

    def start(self, now):
        self.start_laps = self.sup.laps
        self.sup.get_logger().info(
            f"{self.event}: running until {self.target_laps} lap(s) "
            f"({self.target_laps + self.LAUNCH_CROSSING} gate crossings incl. the launch)")

    def tick(self, now):
        if self.done:
            return
        if self.sup.laps - self.start_laps >= self.target_laps + self.LAUNCH_CROSSING:
...
class MissionSupervisor(Node):
    ...
    # Nodes that must be alive for each mission
    MISSION_REQUIRED_NODES = {
        AMI_ACCELERATION: ['pure_pursuit_controller'],
        AMI_SKIDPAD: ['pure_pursuit_controller'],
        AMI_AUTOCROSS: ['pure_pursuit_controller'],
        AMI_TRACK_DRIVE: ['pure_pursuit_controller'],
    }
    ...
    def __init__(self):
        ...
        self.declare_parameter('acceleration_run_s', 15.0)
        self.declare_parameter('skidpad_laps', 4)
        self.declare_parameter('autocross_laps', 0)  # TEMP TEST: 0 + LAUNCH_CROSSING = finishes on the launch crossing. Restore to 1 for a real run.
        ...
        # Subscribers. Own MutuallyExclusive group + MultiThreadedExecutor (see main):
        # tick() can block for up to 2s in stop_planning_control()'s p.wait(),
        # and on a single-threaded spin that starves these callbacks, freezing
        # last_vcu2ai_time -> spurious "/vcu2ai stale (AI_COMMS_LOST risk)" and
        # spurious "left AS_DRIVING" mission aborts while the CAN bridge is fine.
        # MutuallyExclusive (not Reentrant): a separate group is what prevents
        # starvation; reentrancy would let 95Hz /vcu2ai callbacks overlap and
        # interleave/clobber as_state with out-of-order messages.
        ...
        # Process management for pure_pursuit
        self.planning_procs = []
        self.dynamic_amis = {
            self.AMI_ACCELERATION,
            self.AMI_SKIDPAD,
            self.AMI_AUTOCROSS,
            self.AMI_TRACK_DRIVE,
        }
    ...
    def start_planning_control(self):
        """Launch pure_pursuit (hard_path.py is expected to be already running)."""
        if self.planning_procs:
            return

        # pure_pursuit with topic remap so it feeds wheel_speed_controller
        cmd = [
            'ros2', 'run', 'control', 'pure_pursuit.py',
            '--ros-args',
            '-r', '/ackermann_cmd:=/ackermann_cmd_controller',
            # Optional: load the yaml if you want the exact humble defaults
            # '--params-file', os.path.expanduser(
            #     '~/colcon_ws/install/control/share/control/config/ppp.yaml'),
        ]
        try:
            p = subprocess.Popen(cmd)
            self.planning_procs.append(p)
            self.get_logger().info('Started pure_pursuit_controller (remapped to /ackermann_cmd_controller)')
        except Exception as e:
            self.get_logger().error(f'Failed to start pure_pursuit: {e}')

    def stop_planning_control(self):
        if not self.planning_procs:
            return  # nothing running; avoid 10Hz "Stopped ..." log spam
        for p in self.planning_procs:
            if p.poll() is None:
                ...
                try:
                    ...
                    p.kill()
        self.planning_procs.clear()
        self.get_logger().info('Stopped pure_pursuit_controller')
    ...
    def tick(self):
        ...
        # announce the selected mission as soon as it appears on the AMI
        ...
        # We are in AS_DRIVING
        if self.active is None and not self.mission_done:
            handler = self.make_handler(self.ami_state)
            if handler is None:
                return
            # No pre-flight node check here: the only required node for the
            # dynamic events is pure_pursuit, which WE launch two lines below,
            # so it is always "missing" at this instant. Reporting it produced a
            # bogus ERROR on every trackdrive start. health_tick() checks it once
            # the mission is actually running, which is the useful signal.

            # Launch pure_pursuit only for dynamic missions
            if self.ami_state in self.dynamic_amis:
                self.start_planning_control()

            self.active = handler
            self.get_logger().info(
                f"AS_DRIVING with AMI {self.ami_state}: starting mission '{handler.name}'"
            )
    ...
    def missing_nodes(self, ami_state):
        required = self.MISSION_REQUIRED_NODES.get(ami_state, [])
    ...
    def health_tick(self):
        ...
        # Only meaningful while a mission is actually running: the supervisor
        # starts pure_pursuit itself at AS_DRIVING, so before that it is missing
        # by design and warning about it is pure noise. During a mission it is a
        # real alarm (the controller died under us).
        if self.active is not None:
