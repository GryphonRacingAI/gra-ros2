...
class MissionSupervisor(Node):
    ...
    # Nodes that must be alive for each mission (scripted missions only need
    # the CAN bridge chain)
    MISSION_REQUIRED_NODES = {
        AMI_ACCELERATION: ['predict_node', 'track_pathfinder', 'mppi_controller'],
        AMI_SKIDPAD: ['predict_node', 'track_pathfinder', 'mppi_controller', 'lap_counter'],
        AMI_AUTOCROSS: ['predict_node', 'track_pathfinder', 'mppi_controller', 'lap_counter'],
        AMI_TRACK_DRIVE: ['predict_node', 'track_pathfinder', 'mppi_controller', 'lap_counter'],
    }
    ...
    def __init__(self):
        ...
        self.get_logger().info("MissionSupervisor initialized, waiting for AS_DRIVING and a mission from the AMI...")
    ...
    def tick(self):
        ...
        if self.active is None and not self.mission_done:
            ...
            if missing:
                self.get_logger().error(f"starting '{handler.name}' with missing nodes: {', '.join(missing)}")
            self.active = handler
